...
{
   // uint8_t out[2] = { reg, 0xff }; this is original
   uint8_t out[2] = { reg & 0x7F, 0x00 }; // Read command: MSB should be 0 // chatgpt suggested this
   uint8_t in[2];
...
lora_init(void)
{
   ...
   while(i++ < TIMEOUT_RESET) {
      ...
      if (version != 0x12) {
         ESP_LOGE("LoRa", "LoRa chip not found! Version: 0x%02X", version);
         return false; // ⛔ Important!
      }
   
     ESP_LOGI("LoRa", "LoRa chip detected successfully!");
      vTaskDelay(2);
   }
